...
export async function GitHubActivity() {
  ...
  return (
    <section
      className="mx-auto grid min-h-60 w-full max-w-[var(--data-width)] grid-cols-1 border-x border-b border-[var(--cobalt)] bg-[var(--ledger-surface)] tabular-nums"
      aria-labelledby="activity-title"
    >
      <header className="flex min-w-0 items-center justify-between gap-6 border-b border-[var(--cobalt)] px-5 py-4 min-[641px]:px-6">
        <div className="flex min-w-0 flex-wrap items-center gap-x-5 gap-y-2">
          ...
          <a
            className="inline-flex items-center gap-2 font-mono [font-size:var(--type-label)] text-[var(--ink)] no-underline hover:text-[var(--cobalt)] hover:underline [&_svg]:size-[1.15rem]"
      ...
      <div
        className="grid min-w-0 grid-cols-1 min-[641px]:grid-cols-[minmax(14rem,0.9fr)_minmax(0,4.1fr)]"
        role="region"
        aria-label="GitHub account statistics"
      >
        <a
          className="grid min-h-[6.5rem] grid-cols-[3rem_minmax(0,1fr)] grid-rows-[auto_auto] items-center gap-x-[0.8rem] gap-y-1 border-b border-[var(--cobalt)] px-5 py-4 text-[var(--ink)] no-underline transition-colors duration-[180ms] hover:bg-[var(--paper-raised)] focus-visible:-outline-offset-4 min-[641px]:min-h-[6.75rem] min-[641px]:border-r min-[641px]:border-b-0 min-[641px]:px-6"
          href={activity.profileUrl}
          target="_blank"
          rel="me noopener noreferrer"
          ...
          {metrics.map((metric) => (
            ...
            >
              ...
              <dd className="m-0 flex min-w-0 flex-col items-start gap-1 min-[641px]:flex-row min-[641px]:items-baseline min-[641px]:gap-[0.45rem]">
                <strong className="overflow-hidden text-ellipsis font-mono text-[clamp(1.55rem,8vw,2.25rem)] leading-[0.9] font-semibold tracking-[-0.03em] text-[var(--ink)] min-[641px]:text-[clamp(1.75rem,3vw,2.25rem)]">
